...
namespace SunTimer
{
    public class sunCheck
    {

        // make the sunrise a public item
        public DateTime sunrise;

        // make the sunset a public item
        public DateTime sunset;

        // make the isSunrise a public item
        public bool isSunrise;

        // make the isSunset a public item
        public bool isSunset;
        
        // make the checkDate a public item
        private DateTime checkDateField = DateTime.Now;
        ...
        // make the lat a public item
        private double latField = 0.0;
        ...
        // make the latDMS a public item
        private LatitudeCoords latFieldDMS;

        public LatitudeCoords latDMS
        {
            get
            {
                return latFieldDMS;
        ...
        // make the lon a public item
        private double lonField = 0.0;
        ...
        // make the lonDMS a public item
        private LongitudeCoords lonFieldDMS;

        public LongitudeCoords lonDMS
        {
            get
            {
                return lonFieldDMS;
        ...
        public bool checkTime()
        {
            lock (mLock)    // lock for thread safety
                {
                // calculate the difference in the local time and utc, in hours
                TimeZoneInfo utcZone = TimeZoneInfo.Utc;
                DateTime zoneTest = Convert.ToDateTime(checkDate.ToString("d") + " 03:14:15 AM");
                DateTime diffFromUtc = TimeZoneInfo.ConvertTime(zoneTest, utcZone);
                double zone = diffFromUtc.Subtract(zoneTest).Hours;

                double jd = GetJulianDay(checkDate) - 2451545;  // Julian day relative to Jan 1.5, 2000

                    if ((Sign(zone) == Sign(lon)) && (zone != 0))
                    {
//                        Debug.WriteLine("WARNING: time zone and longitude are incompatible!");
                        return false;
                    }
                    ...
                    sunrise = new DateTime(checkDate.Year, checkDate.Month, checkDate.Day, mRiseTimeArr[0], mRiseTimeArr[1], 0);
                    sunset = new DateTime(checkDate.Year, checkDate.Month, checkDate.Day, mSetTimeArr[0], mSetTimeArr[1], 0);
